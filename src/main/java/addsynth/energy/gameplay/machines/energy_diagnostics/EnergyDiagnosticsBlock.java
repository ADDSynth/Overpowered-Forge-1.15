...
public final class EnergyDiagnosticsBlock extends Block {
  ...
  public EnergyDiagnosticsBlock(){
    super(Block.Properties.create(Material.IRON, MaterialColor.WOOL).hardnessAndResistance(1.5f, 6.0f));
  ...
  @Deprecated
  @Override
  public ActionResultType onBlockActivated(BlockState state, World world, BlockPos position, PlayerEntity player, Hand hand, BlockRayTraceResult hit_result){
    if(world.isRemote){
      final TileEnergyDiagnostics tile = MinecraftUtility.getTileEntity(position, world, TileEnergyDiagnostics.class);
      if(tile != null){
        GuiProvider.openEnergyDiagnostics(tile, getTranslationKey());
      }
    }
