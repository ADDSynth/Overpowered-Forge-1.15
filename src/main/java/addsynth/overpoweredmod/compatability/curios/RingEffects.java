...
public enum RingEffects {
  ...
  SPEED          ( 1, "Speed",           Effects.SPEED,            true),
  STRENGTH       ( 2, "Strength",        Effects.STRENGTH,         true),
  HASTE          ( 3, "Haste",           Effects.HASTE,            true),
  LUCK           ( 4, "Luck",            Effects.LUCK,             true),
  EXTRA_HEALTH   ( 5, "Extra Health",    Effects.HEALTH_BOOST,     true),
  JUMP           ( 6, "Jump Boost",      Effects.JUMP_BOOST,       true),
  ...
  public static final void addEffectsToEntity(final ItemStack stack, final LivingEntity livingEntity){
    ...
    if(effect_id > 0){
      ...
      if(ring_effect.has_levels){
        ...
        livingEntity.addPotionEffect(new EffectInstance(ring_effect.effect, Integer.MAX_VALUE, level, false, ring_particles));
      }
      else{
        livingEntity.addPotionEffect(new EffectInstance(ring_effect.effect, Integer.MAX_VALUE, 0, false, ring_particles));
      }
    }
  }
  ...
  public static final void checkEntityHasEffect(final ItemStack stack, final LivingEntity livingEntity){
    ...
    if(effect_id > 0){
      final RingEffects ring_effect = RingEffects.values()[effect_id];
      final EffectInstance effect = livingEntity.getActivePotionEffect(ring_effect.effect);
  ...
  public static final void removeEffectsFromEntity(final ItemStack stack, final LivingEntity livingEntity){
    ...
    if(effect_id > 0){
      final RingEffects ring_effect = RingEffects.values()[effect_id];
      livingEntity.removePotionEffect(ring_effect.effect);
    }
  }
  ...
  public static final void assignToolTip(final ItemStack stack, final List<ITextComponent> tooltip){
    ...
    if(effect_id > 0){
      ...
      if(ring_effect.has_levels){
        ...
        tooltip.add(new StringTextComponent(ring_effect.translation_key + " " + level).applyTextStyle(TextFormatting.GRAY));
      }
      else{
        tooltip.add(new StringTextComponent(ring_effect.translation_key).applyTextStyle(TextFormatting.GRAY));
      }
    }
  }
