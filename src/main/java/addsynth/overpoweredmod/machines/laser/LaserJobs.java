...
public final class LaserJobs {
  ...
  private final static class LaserBeamData {
    ...
    private LaserBeamData(BlockPos position, int color, Direction direction){
      ...
      final int x = position.getX() + direction.getXOffset();
      final int y = position.getY() + direction.getYOffset();
      final int z = position.getZ() + direction.getZOffset();
    ...
    private LaserBeamData(CompoundNBT tag){
      ...
      direction    = Direction.byIndex(tag.getInt("direction"));
    ...
    private final void tick(final World world, final int life){
      ...
      if(move_forward){
        ...
        if(block == Blocks.BEDROCK){
          ...
          world.setBlockState(position, Laser.beams[color].getDefaultState(), Constants.BlockFlags.BLOCK_UPDATE);
        }
        // increment position
    ...
    private LaserJob(final World world, final Collection<BlockPos> positions, final int distance){
      ...
      for(final BlockPos position : positions){
        ...
        if(state.getBlock() instanceof LaserCannon){
          ...
          beams.add(new LaserBeamData(position, block.color, state.get(LaserCannon.FACING)));
        }
      }
    }
  ...
  public static final void tick(final ServerTickEvent event){
    if(event.phase == Phase.START){
      ...
      if(server != null){
        server.getProfiler().startSection(OverpoweredTechnology.MOD_NAME + " Lasers");
        if(laser_jobs.size() > 0){
          for(ServerWorld world : server.getWorlds()){
            for(LaserJob job : laser_jobs){
              if(world.getDimension().getType().getId() == job.dimension){
                job.tick(world);
              }
            }
          }
