...
public final class DimensionalAnchor extends OverpoweredItem {
  ...
  public DimensionalAnchor(){
    super(Names.DIMENSIONAL_ANCHOR, new Item.Properties().group(CreativeTabs.creative_tab).maxStackSize(1));
  }

  public static final boolean player_has_dimensional_anchor(final PlayerEntity player){
    if(Compatibility.CURIOS.loaded){
      if(CuriosAPI.getCurioEquipped(OverpoweredItems.dimensional_anchor, player).isPresent()){
        return true;
      }
    }
    return player.inventory.hasItemStack(new ItemStack(OverpoweredItems.dimensional_anchor));
  }
  ...
  public static final void playerChangingDimension(final EntityTravelToDimensionEvent event){
    if(event.getEntity() instanceof ServerPlayerEntity){
      ...
      if(player_has_dimensional_anchor(player)){
        ...
        final MinecraftServer server = player.getServerWorld().getServer();
