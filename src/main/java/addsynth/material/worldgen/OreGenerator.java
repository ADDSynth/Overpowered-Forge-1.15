...
public final class OreGenerator {
  ...
  private static final void generate_single_ore(final Biome biome, final OreMaterial material, final int tries, final int min, final int max){
    if(valid_min_max_values(material.name, min, max)){
      ...
      final ConfiguredFeature   configured_feature = new ConfiguredFeature<>(Feature.EMERALD_ORE, new ReplaceBlockConfig(Blocks.STONE.getDefaultState(), material.ore.getDefaultState()));
  ...
  private static final void generate_ore(final Biome biome, final OreMaterial material, final int size, final int tries, final int min, final int max){
    if(valid_min_max_values(material.name, min, max)){
      final int min_depth = Math.min(min, max);
      final int max_depth = Math.max(min, max);
      final ConfiguredFeature   configured_feature = new ConfiguredFeature<>(Feature.ORE, new OreFeatureConfig(OreFeatureConfig.FillerBlockType.NATURAL_STONE, material.ore.getDefaultState(), size));
      final ConfiguredPlacement configured_placement = new ConfiguredPlacement<>(Placement.COUNT_RANGE, new CountRangeConfig(tries, min_depth, min_depth, max_depth));
      final DecoratedFeatureConfig decorated_feature = new DecoratedFeatureConfig(configured_feature, configured_placement);
      biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, new ConfiguredFeature<>(Feature.DECORATED, decorated_feature));
    }
